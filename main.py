...
@app.callback(
    Output('atomos', 'figure'),
    [Input('opcion', 'value')]
)
def update_atomo(index):
    traces = []
    ...
    traces.append(go.Scatter3d(
        x=[df1['x'][i] for i in index],
        y=[df1['y'][i] for i in index],
        z=[df1['z'][i] for i in index],
        mode='markers',
        name='Ag Selected',
        marker=dict(
            size=12,
    ...
    fig = {
        'data': traces,
        'layout': go.Layout(
            title='Posiciones de los Atomos de I y Ag'
        )
    }
    return fig
